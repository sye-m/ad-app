...
describe('SpotifyService', () => {
  ...
  describe('searchTrack', () => {
    it('searches a track', 
    inject([SpotifyService,MockBackend], fakeAsync((svc, bck)=> {
      let res;
      expectURL(bck, "https://api.spotify.com/v1/albums/ALBUM_ID");
      svc.searchTrack('TERM').subscribe(resp => res= resp);
      tick();
      expect(res).toBe('felipe');
    }))
   );
  });
